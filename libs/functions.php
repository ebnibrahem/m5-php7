...
/**
 * Get application url inforamtion
 * @return object|null;
 */
function app()
{
	$app = \M5\MVC\App::getRouter();
	return $app;
}
...
function view($page, $data = [], $options = [])
{
	...
	return View::render($page, $data, $options);
}
 ...
 * @return String
 */
function parsePagePath($path, $ext = '')
{
	return str_replace([".", ":"], DS, $path) . $ext;
}
 ...
 * @param Array $options include optional options
 * @return mixed
 */
function template($page, $options = [])
{
	$viewPath = V_PATH;
	$file = $viewPath . parsePagePath($page) . '.php';

	if (!file_exists($file))
		throw new \Exception("<b>$file</b> does not found!", 1);

	extract($options);

	$options['before'] = $before = $options['before'] ? require_once $before : '';
	require_once $file;
	$options['after'] = $after = $options['after'] ? require_once $after : '';
}

/**
 * show image or logo placeholder.
 *
 * @param string $src
 * @param string $storage_path
 * @return String
 */
function avatar($src, $storage_path = '')
{
...
function pa($arr = '', $stop = '', $bg_clr = 'rgba(225,255,255,0.8)')
{
	...
	style="color:#272727;background-color:' . $bg_clr . ';z-index:9999999;position:relative;width:100%;text-align:left;font-size:12px;font-family:Fira Code">';
 ...
 *
 */
function s($string, $separator = "")
{
	$language = app()->getLanguage();

	$anyErrorFound = falSE;

	$lang_dir = ROOT . LANGUAGE_PREFIX . DS . strtolower($language) . DS;

	//parse string param
	$parse_string = explode(".", $string);
	// print_r($parse_string);
	$lang_file = $parse_string[0] . '.php';
	$lang_key = $parse_string[1];
	...
	//get language string from language array;
	$language_strings = require($lang_dir . $lang_file);

	if ($anyErrorFound == true) {
		return $string;
	}

	if (array_key_exists($lang_key, $language_strings)) {
		return $language_strings[$lang_key];
	}
 ...
 * return absolute url
 */
function url_v0($path = '', $flag = null)
{
	return
		!$flag ?
...
{
	$append_lang = ($language_flag = $GLOBALS['language_flag']) ? $language_flag . "/" : "";
	// console([$append_lang]);
	$url = strtolower(URL . $append_lang . $path);
	return $url;
}
 ...
 * @type mixed
 */
function date_echo($date_var, $to = 'Y-m-d')
{
