...
/* 404 error */
Config::set("default_404_class", "Errors");
...
$access_rule_to_admin_dir = function () {
	$login_args = ['redirect' => url(ADMIN_PREFIX), 'msg' => s('g.access_denied')];
	// pa($login_args,1);
...
$access_rule_to_users_dir = function () {
	$login_user_args = ['redirect' => url(), 'msg' => s('g.access_denied'), "session" => "login"];
	// Session::get();
	// pa( M5\Library\Session::get() ,1);
	$redirect_again_to = $_SERVER['REQUEST_URI'];
